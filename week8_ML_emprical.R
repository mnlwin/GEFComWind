...
for (i in 1:10){
  ...
  for (j in 2:6){ 
    ...
    plot(predicted_quant[[i]][[j]][10,])  #only plot the first quantile prediction, should have same shape as quantile
  }
}
print('looks reasonable!')
...
#write.csv(all_quant_preds,file=paste("quantile_est.combined_submit3.csv",sep=""), row.names=FALSE, col.names=FALSE)
...
# y = target variable 
# q = my quantile value, ie, solutions[[1]][i,j]
...
for (i in 1:10){                                #for each wind farm
  loss_farm = 0
  L=0
  for (j in 1:length(solutions[[i]][,1])){      #for each row of predictions
    for (a in 1:99){                            #for each quantile
    
      q = solutions[[i]][j,a+1]                 #target quantile value is equal to quantile prediction value
  ...
  print(paste('avg. pinball loss of farm', i, '=', loss_farm/(99*length(solutions[[i]][,1]))))
  loss_total[i] = loss_farm
}

print(paste('expexcted pinball loss score =', as.character(sum(loss_total)/(10*length(solutions[[i]][,1])*99))), sep = '')
